...
export function ChapterStudyClient({ chapterId }: { chapterId: string }) {
  ...
  // Dynamically generate flashcards for this chapter
  const flashcards = [
    {
      id: 1,
      category: '核心觀念',
      front: `【核心觀念】${textbook.title}之本質與法理核心？`,
      back: textbook.coreConcept,
      tips: '建議理解其立法目的與法律效果'
    },
    ...textbook.importantLaws.map((law, idx) => ({
      id: 2 + idx,
      category: '高頻法條',
      front: `【法條要件】${law.lawName} ${law.articleNo} 之核心規範與必背關鍵字？`,
      back: `📜 法條原文：\n${law.keyClause}\n\n💡 必背關鍵字：${law.coreKeywords?.join('、') || '無'}\n\n白話解析：${law.plainExplanation}`,
      tips: '第一試最常抽換構成要件主體或顛倒原則例外'
    })),
    {
      id: 10,
      category: '易混淆比較',
      front: `【概念比較】${textbook.confusionComparisons.conceptAName} ↔ ${textbook.confusionComparisons.conceptBName} 關鍵差異？`,
      back: textbook.confusionComparisons.rows.map(r => `• 【${r.dimension}】\n  ${textbook.confusionComparisons.conceptAName}：${r.conceptA}\n  ${textbook.confusionComparisons.conceptBName}：${r.conceptB}`).join('\n\n'),
      tips: textbook.confusionComparisons.title
    },
    ...textbook.importantCases.map((cs, cidx) => ({
      id: 20 + cidx,
      category: '重要實務見解',
      front: `【重要實務】${cs.caseNo} (${cs.court}) 之裁判要旨？`,
      back: `🏛️ ${cs.court} ${cs.caseNo}（${cs.date || '實務代表見解'}）：\n\n${cs.summary}`,
      tips: '100% 官方考試重點實務'
    })),
    ...textbook.commonTraps.map((trap, tidx) => ({
      id: 30 + tidx,
      category: '命題陷阱突破',
      front: `【防雷陷阱】本章第 ${tidx + 1} 個高頻命題陷阱與誘答手法？`,
      back: trap,
      tips: '一試二選一關鍵決策點'
    }))
  ];

  const currentCard = flashcards[flashcardIndex] || flashcards[0];
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-navy-900 rounded-2xl p-6 border border-slate-200 dark:border-navy-800 shadow-sm space-y-4">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="flex items-center p-1 bg-slate-100 dark:bg-navy-950 rounded-xl border border-slate-200 dark:border-navy-800 self-start md:self-auto flex-wrap gap-1">
            <button
            ...
            >
              <BookOpen className="w-3.5 h-3.5" /> 正常研讀
            </button>
            <button
            ...
            >
              <Zap className="w-3.5 h-3.5 text-amber-300" /> 重點整理
            </button>
            <button
            ...
            >
              <Clock className="w-3.5 h-3.5" /> 考前速讀
            </button>
            <button
            ...
            >
              🧠 記憶閃卡 ({flashcards.length})
            </button>
          </div>
        </div>

        {/* Mode Description Sub-banner */}
        <div className="text-xs text-slate-500 dark:text-slate-400 pt-2 border-t border-slate-100 dark:border-navy-800 flex items-center justify-between">
          <span>
            {studyMode === 'NORMAL' && '📖 完整 9 層架構教科書模式：涵蓋法條、實務見解、命題手法與比較表。'}
            {studyMode === 'SUMMARY' && '⚡ 高頻精華摘要模式：過濾長篇敘述，專注核心比較表、必背法條與三大陷阱。'}
            {studyMode === 'CRAM' && '⏱️ 考前 60 秒極速衝刺模式：聚焦一分鐘記憶口訣與高頻防雷針。'}
            {studyMode === 'FLASHCARD' && '🧠 互動記憶閃卡模式：透過正反面翻轉卡片自我抽測，強化長效記憶。'}
          </span>
          <span className="text-[11px] font-semibold text-gold-600 dark:text-gold-400">
            切換模式即時調整內容
          </span>
        </div>
      </div>

      {/* ================= MODE 4: INTERACTIVE FLASHCARD MODE ================= */}
      {studyMode === 'FLASHCARD' && (
        <div className="max-w-3xl mx-auto space-y-6 animate-fadeIn">
          <div className="flex items-center justify-between text-xs text-slate-500 dark:text-slate-400">
            <span>
              閃卡進度：第 <strong>{flashcardIndex + 1}</strong> / {flashcards.length} 張
            </span>
          ...
          <div
            ...
            className={`min-h-[300px] p-8 rounded-3xl border-2 cursor-pointer transition-all duration-300 flex flex-col justify-between select-none shadow-xl ${
              isFlipped
                ? 'bg-gradient-to-br from-purple-50 to-indigo-50 dark:from-navy-900 dark:to-indigo-950/40 border-purple-400 dark:border-purple-600'
                : 'bg-white dark:bg-navy-900 border-slate-200 dark:border-navy-800 hover:border-gold-500/60'
            }`}
          >
            <div>
              <div className="flex items-center justify-between text-xs pb-4 border-b border-slate-100 dark:border-navy-800">
                <span className="font-bold text-slate-400">
                  {isFlipped ? '💡 【解析與核心重點】' : '❓ 【思考與測驗題目】'}
                </span>
              ...
              <div className="py-6">
                {isFlipped ? (
                  <div className="text-sm sm:text-base text-slate-800 dark:text-slate-100 whitespace-pre-line leading-relaxed font-sans font-medium">
                    {currentCard.back}
                  </div>
                ) : (
                  <div className="text-base sm:text-lg text-navy-900 dark:text-white font-bold leading-relaxed font-serif">
                    {currentCard.front}
                  </div>
                )}
              </div>
            ...
            <div className="pt-4 border-t border-slate-100 dark:border-navy-800 flex items-center justify-between text-xs text-slate-400">
              <span>{currentCard.tips}</span>
              <span className="font-semibold text-gold-500">
                {isFlipped ? '點擊翻回正面 🔄' : '點擊查看答案 ➔'}
              </span>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between gap-4">
            <button
              ...
              className="px-4 py-2.5 rounded-xl bg-slate-100 dark:bg-navy-800 text-slate-700 dark:text-slate-200 font-bold text-xs hover:bg-slate-200 dark:hover:bg-navy-700 transition flex items-center gap-1.5"
            >
              <ChevronLeft className="w-4 h-4" /> 上一張
            </button>

            <button
              onClick={() => toggleMasterCard(flashcardIndex)}
              className={`px-4 py-2.5 rounded-xl font-bold text-xs transition flex items-center gap-1.5 border ${
                masteredCards[flashcardIndex]
                  ...
                  : 'bg-white dark:bg-navy-900 text-slate-600 dark:text-slate-300 border-slate-200 dark:border-navy-800 hover:border-emerald-500'
              }`}
            >
              <CheckCircle2 className="w-4 h-4" />
              {masteredCards[flashcardIndex] ? '已標記熟練 🟢' : '標記為熟練'}
            </button>

            <button
              onClick={handleNextCard}
              className="px-4 py-2.5 rounded-xl bg-navy-900 dark:bg-gold-500 text-white dark:text-navy-950 font-bold text-xs hover:bg-navy-800 dark:hover:bg-gold-400 transition flex items-center gap-1.5 shadow-sm"
            >
      ...
      {/* ================= MODE 3: CRAM MODE (考前速讀) ================= */}
      {studyMode === 'CRAM' && (
        <div className="space-y-6 animate-fadeIn">
          {/* Layer 9 Cram Banner */}
          <section className="bg-gradient-to-br from-red-500/10 via-amber-500/10 to-navy-900/40 rounded-3xl p-6 sm:p-8 border-2 border-red-400/80 shadow-lg space-y-4">
            <div className="flex items-center gap-2 text-red-600 dark:text-red-400 font-extrabold text-lg border-b border-red-200 dark:border-red-900/60 pb-3">
              <Clock className="w-6 h-6 text-red-500 animate-pulse" />
              考前 60 秒必背速記清單（考前必讀重點）
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs sm:text-sm font-bold text-slate-900 dark:text-slate-100">
              {textbook.oneMinuteSummary.map((sum, sidx) => (
                <div
                  key={sidx}
                  className="p-3.5 rounded-xl bg-white/90 dark:bg-navy-950/90 border border-red-300/60 dark:border-navy-700 flex items-start gap-2.5 shadow-sm"
                >
                  <span className="w-5 h-5 rounded-full bg-red-500 text-white flex items-center justify-center text-xs font-black flex-shrink-0 mt-0.5">
                    {sidx + 1}
                  </span>
                  <span>{sum}</span>
                </div>
          ...
          {/* Common Traps */}
          <section className="bg-white dark:bg-navy-900 rounded-2xl p-6 border-2 border-amber-300 dark:border-amber-800 shadow-sm space-y-3">
            <div className="flex items-center gap-2 text-amber-600 dark:text-amber-400 font-bold text-base border-b border-slate-100 dark:border-navy-800 pb-3">
              <AlertTriangle className="w-5 h-5 text-amber-500" />
              考前三大防雷避坑針
            </div>
            <div className="space-y-2.5">
              {textbook.commonTraps.map((trap, tidx) => (
                <div key={tidx} className="trap-alert-box text-xs sm:text-sm font-semibold">
                  ⚠️ {trap}
                </div>
              ))}
            </div>
      ...
      {studyMode === 'SUMMARY' && (
        <div className="space-y-6 animate-fadeIn">
          ...
          <section className="bg-white dark:bg-navy-900 rounded-2xl p-6 border border-slate-200 dark:border-navy-800 shadow-sm space-y-3">
            ...
            <p className="text-sm text-slate-700 dark:text-slate-200 leading-relaxed font-sans bg-amber-50/40 dark:bg-navy-950/60 p-4 rounded-xl border border-amber-200/60 dark:border-navy-800 font-medium">
              {textbook.coreConcept}
            </p>
          </section>

          {/* Layer 6: Confusion Matrix Comparison Table */}
          <section className="bg-white dark:bg-navy-900 rounded-2xl p-6 border-2 border-purple-400/50 dark:border-purple-800 shadow-sm space-y-4">
            <div className="flex items-center gap-2 text-navy-900 dark:text-white font-bold text-base border-b border-slate-100 dark:border-navy-800 pb-3">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left text-xs sm:text-sm border-collapse">
                <thead>
                  <tr className="bg-purple-50 dark:bg-navy-950 border-b border-purple-200 dark:border-navy-800">
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-navy-800 font-medium">
                  {textbook.confusionComparisons.rows.map((row, ridx) => (
                    <tr key={ridx} className="hover:bg-slate-50/50 dark:hover:bg-navy-950/40">
                      <td className="p-3 font-semibold text-slate-600 dark:text-slate-300 bg-slate-50/50 dark:bg-navy-950/30">
                        {row.dimension}
                      </td>
      ...
      {studyMode === 'NORMAL' && (
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 animate-fadeIn">
          ...
          <div className="lg:col-span-3 space-y-6">
            ...
            <section id="layer-1" className="bg-white dark:bg-navy-900 rounded-2xl p-6 border border-slate-200 dark:border-navy-800 shadow-sm space-y-3 scroll-mt-24">
              <div className="flex items-center gap-2 text-navy-900 dark:text-white font-bold text-base border-b border-slate-100 dark:border-navy-800 pb-3">
            ...
            <section id="layer-2" className="bg-white dark:bg-navy-900 rounded-2xl p-6 border border-slate-200 dark:border-navy-800 shadow-sm space-y-3 scroll-mt-24">
              <div className="flex items-center gap-2 text-navy-900 dark:text-white font-bold text-base border-b border-slate-100 dark:border-navy-800 pb-3">
                <span className="flex items-center justify-center w-6 h-6 rounded-lg bg-gold-100 dark:bg-gold-950 text-gold-700 dark:text-gold-300 text-xs font-black">
                  2
                </span>
            ...
            <section id="layer-3" className="bg-white dark:bg-navy-900 rounded-2xl p-6 border border-slate-200 dark:border-navy-800 shadow-sm space-y-4 scroll-mt-24">
              <div className="flex items-center justify-between border-b border-slate-100 dark:border-navy-800 pb-3">
                <div className="flex items-center gap-2 text-navy-900 dark:text-white font-bold text-base">
                  <span className="flex items-center justify-center w-6 h-6 rounded-lg bg-gold-100 dark:bg-gold-950 text-gold-700 dark:text-gold-300 text-xs font-black">
                    3
            ...
            <section id="layer-4" className="bg-white dark:bg-navy-900 rounded-2xl p-6 border border-slate-200 dark:border-navy-800 shadow-sm space-y-3 scroll-mt-24">
              <div className="flex items-center gap-2 text-navy-900 dark:text-white font-bold text-base border-b border-slate-100 dark:border-navy-800 pb-3">
                <span className="flex items-center justify-center w-6 h-6 rounded-lg bg-gold-100 dark:bg-gold-950 text-gold-700 dark:text-gold-300 text-xs font-black">
                  4
                </span>
            ...
            <section id="layer-5" className="bg-white dark:bg-navy-900 rounded-2xl p-6 border border-slate-200 dark:border-navy-800 shadow-sm space-y-3 scroll-mt-24">
            ...
            <section id="layer-6" className="bg-white dark:bg-navy-900 rounded-2xl p-6 border border-slate-200 dark:border-navy-800 shadow-sm space-y-4 scroll-mt-24">
              <div className="flex items-center gap-2 text-navy-900 dark:text-white font-bold text-base border-b border-slate-100 dark:border-navy-800 pb-3">
            ...
            {/* Layer 7: Important Cases (重要實務見解) */}
            <section id="layer-7" className="bg-white dark:bg-navy-900 rounded-2xl p-6 border border-slate-200 dark:border-navy-800 shadow-sm space-y-4 scroll-mt-24">
              <div className="flex items-center justify-between border-b border-slate-100 dark:border-navy-800 pb-3">
                <div className="flex items-center gap-2 text-navy-900 dark:text-white font-bold text-base">
            ...
            <section id="layer-8" className="bg-white dark:bg-navy-900 rounded-2xl p-6 border border-slate-200 dark:border-navy-800 shadow-sm space-y-3 scroll-mt-24">
              <div className="flex items-center gap-2 text-navy-900 dark:text-white font-bold text-base border-b border-slate-100 dark:border-navy-800 pb-3">
                <span className="flex items-center justify-center w-6 h-6 rounded-lg bg-gold-100 dark:bg-gold-950 text-gold-700 dark:text-gold-300 text-xs font-black">
                  8
                </span>
            ...
            <section id="layer-9" className="bg-gradient-to-br from-gold-500/10 via-amber-500/5 to-navy-900/40 rounded-2xl p-6 border-2 border-gold-400/60 dark:border-gold-500/50 shadow-md space-y-3 scroll-mt-24">
